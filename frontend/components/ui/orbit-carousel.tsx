...
export interface OrbitCarouselProps {
  ...
  onConnectClick?: (person: Person) => void;
}
...
export default function OrbitCarousel({
  ...
  containerRadius: customContainerRadius,
  profileSize: customProfileSize,
  onPersonClick,
  onConnectClick,
}: OrbitCarouselProps) {
  ...
  const containerRadius = customContainerRadius ?? (isMobile ? 130 : 200);
  const profileSize = customProfileSize ?? (isMobile ? 60 : 80);
  const containerSize = containerRadius * 2 + 100;

  // Calculate rotation for each profile
  const getRotation = React.useCallback(
    (index: number): number => (index - activeIndex) * (360 / data.length),
    [activeIndex, data.length]
  );

  // Navigation
  const next = () => setActiveIndex((i) => (i + 1) % data.length);
  const prev = () => setActiveIndex((i) => (i - 1 + data.length) % data.length);

  const handleProfileClick = React.useCallback((index: number) => {
    if (index === activeIndex) return;
    setActiveIndex(index);
    if (onPersonClick) {
      onPersonClick(data[index]);
    }
  }, [activeIndex, data, onPersonClick]);

  const handleConnectClick = () => {
    if (onConnectClick) {
      onConnectClick(data[activeIndex]);
    }
  };
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-center text-xs md:text-sm text-gray-600 dark:text-gray-400 mt-1">
                ...
                <span className="truncate">{data[activeIndex].role}</span>
              </div>
              <div className="flex items-center justify-center text-xs text-gray-500 dark:text-gray-500 mt-0.5">
                <Mail size={12} className="mr-1" /> 
                <span className="truncate">{data[activeIndex].email}</span>
              </div>
            </motion.div>
            <motion.div 
              initial={{ opacity: 0 }}
